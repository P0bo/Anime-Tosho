...
def parse_xml(xml_content):
    ...
    for item_text in items:
        ...
        if item_data['title'] and item_data['link'] and item_data['guid'] and item_data['pubDate'] and item_data['size'] and item_data['seeders'] and item_data['leechers'] and item_data['downloads']:
...
def create_xml_entries(feed_name, feed_link, entries, existing_ids):
    ...
    for entry in entries:
        ...
        trusted = entry.get('trusted', 'No')
        remake = entry.get('remake', 'No')

        hyperlink = f'<a href="{entry["guid"]}">{title}</a>'
        nyaa_id = entry_id
        description_content = (
            f"{size_mb:.2f} MiB | Seeders: {seeders} | Leechers: {leechers} | "
            f"Downloads: {downloads} | Nyaa: {nyaa_id} | Trusted: {trusted} | Remake: {remake} | {hyperlink}"
        )
